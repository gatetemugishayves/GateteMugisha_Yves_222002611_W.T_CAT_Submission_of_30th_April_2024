...
<section>
    ...
    <form method="post">
        ...
        <input type="text" id="rl" name="rl" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO staff(staff_id, name, role, email , phone) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $sid, $n, $rl, $e, $tel);
...
<?php
include('database_connection.php');
// SQL query to fetch data from the Staff table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of Staff</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Staff</h2></center>
    <table border="5">
        <tr>
            <th>staff_id</th>
            <th>name</th>
        ...
        // Prepare SQL query to retrieve all Staff
        $sql = "SELECT * FROM staff";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $sid = $row['staff_id']; // Fetch the staff_id
                echo "<tr>
                    <td>" . $row['staff_id'] . "</td>
                    <td>" . $row['name'] . "</td>
                    <td>" . $row['role'] . "</td>
                    ...
                    <td><a style='padding:4px' href='delete_staff.php?staff_id=$sid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_staff.php?staff_id=$sid'>Update</a></td> 
                </tr>";
            }
        } else {
